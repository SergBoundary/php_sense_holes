...
$query_count_events = "SELECT Count(cn.content_id) AS count
					FROM contents AS cn, content_relations_direction AS crd, content_relations_status AS crs 
					WHERE cn.content_id = crs.content_id AND cn.content_id = crd.content_id AND crs.content_status_id = '1' AND cn.content_language = '".$language_use."' AND crd.direction_id = '".$direction."'";
$result_count_events = mysqli_query($db, $query_count_events) or die(mysqli_error($db));
if (mysqli_num_rows($result_count_events) == 1) {
	$row_count_events = mysqli_fetch_array($result_count_events, MYSQL_ASSOC);
	$count_events = $row_count_events['count'];
} else {
	die(mysqli_error($db));
}

// Подсчет контента аннотаций
$query_count_annotations = "SELECT Count(cn.content_id) AS count
					FROM contents AS cn, content_relations_direction AS crd, content_relations_status AS crs 
					WHERE cn.content_id = crs.content_id AND cn.content_id = crd.content_id AND crs.content_status_id = '2' AND cn.content_language = '".$language_use."' AND crd.direction_id = '".$direction."'";
$result_count_annotations = mysqli_query($db, $query_count_annotations) or die(mysqli_error($db));
if (mysqli_num_rows($result_count_annotations) == 1) {
	$row_count_annotations = mysqli_fetch_array($result_count_annotations, MYSQL_ASSOC);
	$count_annotations = $row_count_annotations['count'];
} else {
	die(mysqli_error($db));
}
...
$query_count_criticism_categories = "SELECT Count(content_category_id) AS count FROM content_categories WHERE content_category_id IN (
					...
					AND crs.content_status_id = '3' AND cn.content_language = '".$language_use."' AND crd.direction_id = '".$direction."')";
$result_count_criticism_categories = mysqli_query($db, $query_count_criticism_categories) or die(mysqli_error($db));
if (mysqli_num_rows($result_count_criticism_categories) == 1) {
	$row_count_criticism_categories = mysqli_fetch_array($result_count_criticism_categories, MYSQL_ASSOC);
	$count_criticism_categories = $row_count_criticism_categories['count'];
} else {
	die(mysqli_error($db));
}

// Подсчет авторов контента критики 
$query_count_criticism_authors = "SELECT Count(content_author_id) AS count FROM content_authors WHERE content_author_id IN (
					SELECT ca.content_author_id
					FROM content_authors AS ca, content_relations_author AS cra, content_relations_direction AS crd, content_relations_status AS crs 
					WHERE cra.content_id = crd.content_id AND cra.content_id = crs.content_id AND ca.content_author_id = cra.content_author_id 
					AND crs.content_status_id = '3' AND ca.content_author_language = '".$language_use."' AND crd.direction_id = '".$direction."')";
$result_count_criticism_authors = mysqli_query($db, $query_count_criticism_authors) or die(mysqli_error($db));
if (mysqli_num_rows($result_count_criticism_authors) == 1) {
	$row_count_criticism_authors = mysqli_fetch_array($result_count_criticism_authors, MYSQL_ASSOC);
	$count_criticism_authors = $row_count_criticism_authors['count'];
} else {
	die(mysqli_error($db));
}

// Подсчет типов контента критики 
$query_count_criticism_types = "SELECT Count(content_type_id) AS count FROM content_types WHERE content_type_id IN (
					...
					AND crs.content_status_id = '3' AND cn.content_language = '".$language_use."' AND crd.direction_id = '".$direction."')";
$result_count_criticism_types = mysqli_query($db, $query_count_criticism_types) or die(mysqli_error($db));
if (mysqli_num_rows($result_count_criticism_types) == 1) {
	$row_count_criticism_types = mysqli_fetch_array($result_count_criticism_types, MYSQL_ASSOC);
	$count_criticism_types = $row_count_criticism_types['count'];
} else {
	die(mysqli_error($db));
}

// Подсчет критического контента
$query_count_criticism = "SELECT Count(cn.content_id) AS count
					FROM contents AS cn, content_relations_direction AS crd, content_relations_status AS crs 
					WHERE cn.content_id = crs.content_id AND cn.content_id = crd.content_id AND crs.content_status_id = '3' AND cn.content_language = '".$language_use."' AND crd.direction_id = '".$direction."'";
$result_count_criticism = mysqli_query($db, $query_count_criticism) or die(mysqli_error($db));
if (mysqli_num_rows($result_count_criticism) == 1) {
	$row_count_criticism = mysqli_fetch_array($result_count_criticism, MYSQL_ASSOC);
	$count_criticism = $row_count_criticism['count'];
} else {
	die(mysqli_error($db));
}

// Подсчет категорий контента библиотеки
$query_count_library_categories = "SELECT Count(content_category_id) AS count FROM content_categories WHERE content_category_id IN (
					SELECT cc.content_category_id
					FROM content_categories AS cc, contents AS cn, content_relations_category AS crc, content_relations_direction AS crd, content_relations_status AS crs 
					WHERE cc.content_category_id = crc.content_category_id AND crc.content_id = cn.content_id AND cn.content_id = crd.content_id AND cc.direction_id = crd.direction_id AND crc.content_id = crs.content_id
					AND crs.content_status_id = '4' AND cn.content_language = '".$language_use."' AND crd.direction_id = '".$direction."')";
$result_count_library_categories = mysqli_query($db, $query_count_library_categories) or die(mysqli_error($db));
if (mysqli_num_rows($result_count_library_categories) == 1) {
	$row_count_library_categories = mysqli_fetch_array($result_count_library_categories, MYSQL_ASSOC);
	$count_library_categories = $row_count_library_categories['count'];
} else {
	die(mysqli_error($db));
}

// Подсчет авторов контента библиотеки 
$query_count_library_authors = "SELECT Count(content_author_id) AS count FROM content_authors WHERE content_author_id IN (
					SELECT ca.content_author_id
					FROM content_authors AS ca, content_relations_author AS cra, content_relations_direction AS crd, content_relations_status AS crs 
					WHERE cra.content_id = crd.content_id AND cra.content_id = crs.content_id AND ca.content_author_id = cra.content_author_id 
					AND crs.content_status_id = '4' AND ca.content_author_language = '".$language_use."' AND crd.direction_id = '".$direction."')";
$result_count_library_authors = mysqli_query($db, $query_count_library_authors) or die(mysqli_error($db));
if (mysqli_num_rows($result_count_library_authors) == 1) {
	$row_count_library_authors = mysqli_fetch_array($result_count_library_authors, MYSQL_ASSOC);
	$count_library_authors = $row_count_library_authors['count'];
} else {
	die(mysqli_error($db));
}

// Подсчет типов контента библиотеки 
$query_count_library_types = "SELECT Count(content_type_id) AS count FROM content_types WHERE content_type_id IN (
					SELECT ct.content_type_id
					FROM contents AS cn, content_types AS ct, content_relations_direction AS crd, content_relations_status AS crs, content_relations_type AS crt 
					WHERE cn.content_id = crd.content_id AND cn.content_id = crs.content_id AND cn.content_id = crt.content_id AND ct.content_type_id = crt.content_type_id 
					AND crs.content_status_id = '4' AND cn.content_language = '".$language_use."' AND crd.direction_id = '".$direction."')";
$result_count_library_types = mysqli_query($db, $query_count_library_types) or die(mysqli_error($db));
if (mysqli_num_rows($result_count_library_types) == 1) {
	$row_count_library_types = mysqli_fetch_array($result_count_library_types, MYSQL_ASSOC);
	$count_library_types = $row_count_library_types['count'];
} else {
	die(mysqli_error($db));
}

// Подсчет библиотечного контента
$query_count_library = "SELECT Count(cn.content_id) AS count
					FROM contents AS cn, content_relations_direction AS crd, content_relations_status AS crs 
					WHERE cn.content_id = crs.content_id AND cn.content_id = crd.content_id AND crs.content_status_id = '4' AND cn.content_language = '".$language_use."' AND crd.direction_id = '".$direction."'";
$result_count_library = mysqli_query($db, $query_count_library) or die(mysqli_error($db));
if (mysqli_num_rows($result_count_library) == 1) {
	$row_count_library = mysqli_fetch_array($result_count_library, MYSQL_ASSOC);
	$count_library = $row_count_library['count'];
} else {
	die(mysqli_error($db));
}
				...
				if ($aside == 2) {
					...
					case '2':
						if ($count_criticism_authors != 0) {
							...
							$i = 0;
							// Загрузка авторов
							$query_author = "SELECT DISTINCT Concat(at.content_author_surname, ' ', at.content_author_name) AS content_author, Count(at.content_author_id) AS count_author
								FROM content_authors AS at, contents AS cn, content_relations_author AS cra, content_relations_direction AS crd, content_relations_status AS crs 
								WHERE at.content_author_id = cra.content_author_id AND cra.content_id = cn.content_id AND cra.content_id = crd.content_id AND cra.content_id = crs.content_id  
								AND crs.content_status_id = '".$content_status."' AND cn.content_language = '".$language_use."' AND crd.direction_id = '".$direction."' 
					...
					case '2':
						if ($count_library_authors != 0) {
							...
							$i = 0;
							// Загрузка авторов
							$query_author = "SELECT Concat(at.content_author_surname, ' ', at.content_author_name) AS content_author, Count(at.content_author_id) AS count_author
								FROM content_authors AS at, contents AS cn, content_relations_author AS cra, content_relations_direction AS crd, content_relations_status AS crs 
								WHERE at.content_author_id = cra.content_author_id AND cra.content_id = cn.content_id AND cra.content_id = crd.content_id AND cra.content_id = crs.content_id  
								AND crs.content_status_id = '".$content_status."' AND cn.content_language = '".$language_use."' AND crd.direction_id = '".$direction."' 
								GROUP BY content_author
